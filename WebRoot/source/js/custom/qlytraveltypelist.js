...
$(document).ready(function(){
	...
	$('#a_del_batch').bind('click', function() {$("input[name='ck']").filter(function(i) {return $(this).attr('checked');}).each(function() {c=1;});if (c > 0) {if( confirm('您确定要删除吗?') ){$('#listForm').attr('action', 'qlytraveltype!batchdel.do').submit();}} else {alert('没有选中');}});
});
